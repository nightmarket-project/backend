...
import static store.nightmarket.domain.payment.service.dto.RejectPaymentDomainServiceDto.*;


class RejectPaymentDomainServiceTest {

    private RejectPaymentDomainService service;
    private SoftAssertions softly;

    @BeforeEach
    void setUp() {
        service = new RejectPaymentDomainService();
        softly = new SoftAssertions();
    }

    @Test
    @DisplayName("모든 DetailPayment가 SUBMITTED 상태일 때 REJECTED 상태로 전이되어야 한다")
    void shouldTransitionAllDetailPaymentsFromSubmittedToRejected() {
        // given
        ...
        executePaymentRecord.getDetailPaymentRecordList()
                .forEach(detailPaymentRecord ->
                        softly.assertThat(detailPaymentRecord.getState())
                                .isEqualTo(DetailPaymentState.REJECTED)
                );
    }

    @ParameterizedTest(name = "현재 상태가 {0}일 때 REJECTED 전이 시도 시 예외 발생")
    @MethodSource("invalidTransitions")
    @DisplayName("유효하지 않은 상태에서 REJECTED로 전이 시 PaymentException 발생해야 함")
    void shouldThrowExceptionWhenTransitionToRejectedFromInvalidState(DetailPaymentState currentState) {
        // given
        PaymentRecord testPayment = PaymentTestUtil.createTestPayment(
                UUID.randomUUID(),
                UUID.randomUUID(),
